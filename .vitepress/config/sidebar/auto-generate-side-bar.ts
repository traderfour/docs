...
import { Options } from "./auto-generate-side-bar";
...
export const getSidebar = (options: Options = {}) => {
  ...
  const dir = fs
    .readdirSync(options.contentRoot)
    .filter(
      (file: string) =>
        (options.contentDirs === null ||
          options.contentDirs?.indexOf(file) !== -1) &&
        fs.statSync(path.join(options.contentRoot, file)).isDirectory()
    );
